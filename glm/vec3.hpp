...
public:
    vec() : x(), y(), z() { /*void*/ }
    vec(const_reference scalar) : x(scalar), y(scalar), z(scalar) { /*void*/ }
    vec(const_reference vx, const_reference vy, const_reference vz) :
        x(vx), y(vy), z(vz) { /*void*/ }

    template<typename X>
    vec(const vec<2, X>& v, const_reference vz = value_type()) :
