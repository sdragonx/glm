...
GLM_INS_API void mat4_mul_impl(const float* m1, const float* m2, float* dest)
{
...
GLM_INS_API void mat4_transform_impl(const float* m1, const float* m2, float* dest)
{
    __m128 a1 = _mm_loadu_ps(m1 + 0);
    __m128 a2 = _mm_loadu_ps(m1 + 4);
    __m128 a3 = _mm_loadu_ps(m1 + 8);
...
GLM_INS_API void mat4_transform_impl(const float* m, const float* v, float* dest)
{
    __asm {
        mov eax, m
        mov ebx, v
...
GLM_API mat<4, 4, float> mat4_mul(const mat<4, 4, float>& m1, const mat<4, 4, float>& m2)
{
    ...
    mat4_mul_impl(m1.data(), m2.data(), product.data());
    return product;
}
...
{
    vec<4, float> product;
    mat4_transform_impl(m.data(), v.data(), product.data());
    return product;
}
#endif
